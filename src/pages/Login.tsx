    ...
    <div className="min-h-screen flex flex-col lg:flex-row" style={{ background: "var(--gradient-bg)" }}>
      <div className="gradient-primary px-6 pt-16 pb-12 rounded-b-[3rem] text-center lg:rounded-none lg:w-1/2 lg:flex lg:flex-col lg:items-center lg:justify-center lg:pb-16">
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
          ...
          <p className="text-secondary/60 text-sm lg:text-base mt-1">Premium Pre-Owned Fashion</p>
          <p className="hidden lg:block text-secondary/40 text-sm mt-4 max-w-xs mx-auto">Buy & sell designer clothes at up to 80% off. Join thousands of fashion lovers on MadFot.</p>
        </motion.div>
      </div>
      ...
      >
        <div className="glass-card rounded-3xl p-6 md:p-8 shadow-card border border-border/30 w-full max-w-md">
          <h2 className="text-xl md:text-2xl font-bold text-foreground mb-1 font-serif">Welcome Back</h2>
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            Don't have an account? <Link to="/signup" className="text-secondary font-semibold hover:underline">Sign Up</Link>
          </p>
        </div>
      </motion.div>
    </div>
